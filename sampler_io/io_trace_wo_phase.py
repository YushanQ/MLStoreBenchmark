...
static __always_inline struct file_info get_file_info(int fd) {
    ...
    bpf_probe_read(&info.offset, sizeof(info.offset), &file->f_pos);
    bpf_probe_read(&f_path, sizeof(f_path), &file->f_path);
    
    pathname = d_path(&f_path, info.path, sizeof(info.path));
    if (!IS_ERR(pathname)) {
        bpf_probe_read_str(info.path, sizeof(info.path), pathname);
    }
...
int syscall__read_enter(struct pt_regs *ctx, int fd, void *buf, size_t count) {
    ...
    data.fd = fd;
    data.size = count;
    file_info = get_file_info(fd);
    data.offset = file_info.offset;
    bpf_probe_read_str(data.fname, sizeof(data.fname), file_info.path);
...
class IOTracker:
    ...
    # def update_stats(self, syscall, size, latency):
